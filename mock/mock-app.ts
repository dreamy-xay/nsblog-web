 ...
 * @LastEditTime: 2023-05-13 15:23:21
 */
 ...
 * @author: dreamy-xay
 */
function lowerObjectKey(obj: Record<string, unknown>): Record<string, unknown> {
...
function getParams(url: string, realUrl: string): Record<string, unknown> {
  ...
  //! 下列代码仅可以匹配 '/:xxxx' 形式路由，无法匹配 '/:xxxx(xxx)' 路由
...
export class Response {
  ...
  /* 状态码对应文字 */
  private statusText: Record<number, string> = {
  ...
  /**
   * @description: 模拟express http返回数据，传递参数返回数据data
   * @param {unknown} data 传递数据 `必传参数`
   * @return {Record<string, unknown>} 返回数据
   * @author: dreamy-xay
   */
  public send(data: unknown): Record<string, unknown> {
    ...
    return {
      ...
      statusText: this.statusText[this.responseStatus]
    };
  }

  /**
   * @description: express json返回数据，传递参数返回数据data
   * @param {Record<string, unknown>} data 传递数据 `必传参数`
   * @return {Record<string, unknown>} 返回数据
   * @author: dreamy-xay
   */
  ...
  /**
   * @description: 模拟express 设置返回状态，传递参数状态码status
   * @param {number} status 状态码 `必传参数`
   * @return {Response} 返回自身，链式调用
   * @author: dreamy-xay
   */
  ...
  /**
   * @description: 模拟express 设置响应头内容
   * @param {string} key 响应头键 `必传参数`
   * @param {unknown} value 响应头值 `必传参数`
   * @return {Response} 返回自身，链式调用
   * @author: dreamy-xay
   */
...
// 缓存请求类型
type CacheRequest = { exp: RegExp; url: string; type: string; callback: (req: Request, res: Response) => any };
...
export class Application {
   ...
   * @author: dreamy-xay
   */
  private request(url: string, type: string, callback: (req: Request, res: Response) => any): void {
   ...
   * @author: dreamy-xay
   */
  public get(url: string, callback: (req: Request, res: Response) => any): void {
   ...
   * @author: dreamy-xay
   */
  public post(url: string, callback: (req: Request, res: Response) => any): void {
   ...
   * @author: dreamy-xay
   */
  public delete(url: string, callback: (req: Request, res: Response) => any): void {
   ...
   * @author: dreamy-xay
   */
  public put(url: string, callback: (req: Request, res: Response) => any): void {
...
export class Server {
  ...
  public listen(): Server {
    ...
    for (const request of sortedAllRequests) {
      Mock.mock(request.exp, request.type, (options: MockCbOptions & { headers: Record<string, string> }) => {
        const req: Request = {
          ...
          path: options.url,
          headers: lowerObjectKey(options.headers)
        };
        ...
        // 如果返回的是 Response 对象则调用 getResponseSendData 方法获取Response的发送数据
        const result: Record<string, unknown> | Response = request.callback(req, res);
        return result instanceof Response ? result.send(null) : result;
      });
    }
    return this;
  }
